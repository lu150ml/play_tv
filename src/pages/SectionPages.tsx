...
function SectionPage({ screen }: { screen: ScreenKey }) {
  ...
  return (
    <div className="mx-auto max-w-canvas">
      ...
      {!selectedCategory ? (
        <>
          <label className="mb-6 flex items-center gap-3 rounded-xl border border-white/10 bg-surface-container px-4 py-3"><Search size={19} className="text-primary"/><input data-focusable="true" enterKeyHint="done" value={query} onKeyDown={(event) => { if (event.key === "Enter") event.currentTarget.blur(); }} onChange={(event) => { setQuery(event.target.value); setPageSize(60); }} placeholder={`Buscar em ${config[screen].title}`} className="w-full bg-transparent outline-none" /></label>
        ...
        <>
          <label className="mb-5 flex items-center gap-3 rounded-xl border border-white/10 bg-surface-container px-4 py-3"><Search size={19} className="text-primary"/><input data-focusable="true" enterKeyHint="done" value={query} onKeyDown={(event) => { if (event.key === "Enter") event.currentTarget.blur(); }} onChange={(event) => { setQuery(event.target.value); setPageSize(60); }} placeholder={`Buscar em ${selectedCategory.title}`} className="w-full bg-transparent outline-none" /></label>
